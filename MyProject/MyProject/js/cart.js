...
`
  ...
  // Fix any incorrect prices in localStorage
  fixProductPrices()

  // Initialize cart count for all pages
  updateCartCount()

  // เพิ่มปุ่มสำหรับเพิ่มสินค้าตัวอย่างในหน้าตะกร้า (สำหรับการทดสอบ)
  if (isCartPage) {
    const cartContainer = document.querySelector(".cart-container")
    if (cartContainer && getCart().length === 0) {
      const addSampleButton = document.createElement("button")
      addSampleButton.className = "add-sample-btn"
      addSampleButton.textContent = "เพิ่มสินค้าตัวอย่างสำหรับทดสอบ"
      addSampleButton.style.margin = "20px auto"
      addSampleButton.style.display = "block"
      addSampleButton.style.padding = "10px 20px"
      addSampleButton.style.backgroundColor = "#3949ab"
      addSampleButton.style.color = "#fff"
      addSampleButton.style.border = "none"
      addSampleButton.style.borderRadius = "4px"
      addSampleButton.style.cursor = "pointer"

      addSampleButton.addEventListener("click", () => {
        addSampleProducts()
      })

      cartContainer.parentNode.insertBefore(addSampleButton, cartContainer)
    }
  }
})
